...
```bash
$ node -r dotenv/config your_script.js dotenv_config_path=/custom/path/to/.env1
```
...
```bash
$ DOTENV_CONFIG_ENCODING=latin1 node -r dotenv/config your_script.js dotenv_config_path=/custom/path/to/.env1
```
...
```js
require('dotenv').config({ path: '/custom/path/to/.env1' })
```
...
const fs = require('fs')
const dotenv = require('dotenv')
const envConfig = dotenv.parse(fs.readFileSync('.env1.override'))
